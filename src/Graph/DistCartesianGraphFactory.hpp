/*
 * DistCartesianGraphFactory.hpp
 *
 *  Created on: Dec 03, 2015
 *      Author: Antonio Leo
 */

#ifndef DISTCARTESIANGRAPHFACTORYOLD_HPP_
#define DISTCARTESIANGRAPHFACTORYOLD_HPP_
...
struct dist_fill_id<dim, G_v, -1, -1>
{
	static inline void fill(G_v & g_v, const grid_key_dx<dim> & gk, const grid_sm<dim, void> & gs)
	{
	}
...
template<unsigned int dim, int loc_id, int glob_id, typename dT, typename G_v, typename v, int impl>
class dist_fill_prop
{
...
public:
	...
	dist_fill_prop(G_v & g_v, const dT (&szd)[dim], grid_key_dx<dim> & gk, const grid_sm<dim, void> & gs) :
...
template<unsigned int dim, int loc_id, int glob_id, typename dT, typename G_v, typename v>
class dist_fill_prop<dim, loc_id, glob_id, dT, G_v, v, 0>
{
...
template<unsigned int dim, int loc_id, int glob_id, typename dT, typename G_v, typename v>
class dist_fill_prop<dim, loc_id, glob_id, dT, G_v, v, 2>
{
...
struct dist_fill_prop_by_type<0, p, Graph, pos...>
{
	enum
	{
		value = 0
...
template<unsigned int dim, int loc_id, int glob_id, int e_sgid, int e_dgid, typename Graph, int se, typename T, unsigned int dim_c, int ... pos>
class DistGraph_constructor_impl
{
public:
	//! Construct Cartesian graph
	static Graph construct(const size_t (&sz)[dim], Box<dim, T> dom, openfpm::vector<idx_t> & vtxdist)
	{
		...
		while (k_it.isNext())
		{
			...
			if (v_id < vtxdist.get(p_id + 1) && v_id >= vtxdist.get(p_id))
			{
				...
				dist_fill_prop<dim, loc_id, glob_id, T, decltype(gp.vertex(local_it)), typename to_boost_vmpl<pos...>::type, dist_fill_prop_by_type<sizeof...(pos), p, Graph, pos...>::value> flp(obj, szd, key, g);
				...
				gp.setGlobalMap(v_id, local_it, local_it);
...
template<unsigned int dim, int loc_id, int glob_id, int e_sgid, int e_dgid, typename Graph, typename T, unsigned int dim_c, int ... pos>
class DistGraph_constructor_impl<dim, loc_id, glob_id, e_sgid, e_dgid, Graph, NO_EDGE, T, dim_c, pos...>
{
public:
	//! Construct Cartesian graph
	static Graph construct(const size_t (&sz)[dim], Box<dim, T> dom, openfpm::vector<idx_t> & vtxdist)
			...
			if (v_id < (size_t)vtxdist.get(p_id + 1) && v_id >= (size_t)vtxdist.get(p_id))
			{
				...
				for (size_t d = dim - 1; d >= dim_c; d--)
				{
					...
					for (size_t j = 0; j < c.size(); j++)
					{
						...
						{
							// Add an edge and set the the edge property to the size of the face (communication weight)
							gp.template addEdge<NoCheck, e_sgid, e_dgid>(start_v, end_v, v_id, end_v);
						}
					}
				}
				++local_it;
...
class DistCartesianGraphFactory
{
...
public:
	...
	static Graph construct(const size_t (&sz)[dim], Box<dim, T> dom, openfpm::vector<idx_t> & vtxdist)
	{
		return DistGraph_constructor_impl<dim, -1, -1, -1, -1, Graph, se, T, dim_c, pos...>::construct(sz, dom, vtxdist);
	}
	 ...
	 *           in contact (in common) between two hyper-cube. NO_EDGE indicate
	 *           no property will store this information
	 * \tparam loc_id property that stores the local vertex id
	 * \tparam glob_id property that stores the local vertex id
	 * \tparam e_sgid property that stores on the edge global id of the source vertex
	 * \tparam e_dgid property that stores on the edge global id of the destination vertex
	 * \tparam T type of the domain like (int real complex ... )
	 * \tparam dim_c Connectivity dimension
	 * \tparam pos... (optional)one or more integer indicating the spatial properties
	 *
	 */
	template<int se, int loc_id, int glob_id, int e_sgid, int e_dgid, typename T, unsigned int dim_c, int ... pos>
	static Graph construct(const size_t (&sz)[dim], Box<dim, T> dom, openfpm::vector<idx_t> & vtxdist)
	{
		return DistGraph_constructor_impl<dim, loc_id, glob_id, e_sgid, e_dgid, Graph, se, T, dim_c, pos...>::construct(sz, dom, vtxdist);
	}
};
