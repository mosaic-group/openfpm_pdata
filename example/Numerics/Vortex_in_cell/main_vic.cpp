...
void init_ring(grid_type & gr, const Box<3,float> & domain)
{
	...
	while (it.isNext())
	{
        ...
        float rad1r  = sqrt((ty-0.5f)*(ty-0.5f) + (tz-0.5f)*(tz-0.5f)) - ringr1*(1.0f + ringnz * noise);
        float rad1t = tx - 0.5f;
...
void helmotz_hodge_projection(grid_type & gr, const Box<3,float> & domain)
{
	...
	while (it.isNext())
	{
        ...
        gr_hl.template get<phi>(key) = 1.0f/gr.spacing(x)/1.0f*(gr.template get<vorticity>(key.move(x,1))[x] - gr.template get<vorticity>(key.move(x,0))[x] ) +
			         	 	 1.0f/gr.spacing(y)/1.0f*(gr.template get<vorticity>(key.move(y,1))[y] - gr.template get<vorticity>(key.move(y,0))[y] ) +
							 1.0f/gr.spacing(z)/1.0f*(gr.template get<vorticity>(key.move(z,1))[z] - gr.template get<vorticity>(key.move(z,0))[z] );
...
/*	petsc_solver<double> solver;
	...
	solver.setAbsTol(0.000001);
	solver.setRestart(150);
	...
	// Give to the solver A and b, return x, the solution
	auto phi_s = solver.solve(A,fd.getB(),SOLVER_PRINT_DETERMINANT | SOLVER_PRINT_RESIDUAL_NORM_INFINITY);
	...
	while (it2.isNext())
	{
		...
		gr.template get<vorticity>(key)[x] -= 1.0f/1.0f/gr.spacing(x)*(gr_hl.template get<phi>(key.move(x,0))-gr_hl.template get<phi>(key.move(x,-1)));
		gr.template get<vorticity>(key)[y] -= 1.0f/1.0f/gr.spacing(y)*(gr_hl.template get<phi>(key.move(y,0))-gr_hl.template get<phi>(key.move(y,-1)));
		gr.template get<vorticity>(key)[z] -= 1.0f/1.0f/gr.spacing(z)*(gr_hl.template get<phi>(key.move(z,0))-gr_hl.template get<phi>(key.move(z,-1)));
...
void comp_vel(Box<3,float> & domain, grid_type & g_vort,grid_type & g_vel, petsc_solver<double>::return_type (& phi_s)[3])
{
	...
	float inv_dx = 1.0f / g_vort.spacing(0);
	float inv_dy = 1.0f / g_vort.spacing(1);
	float inv_dz = 1.0f / g_vort.spacing(2);
...
//		MatIsSymmetric(A.getMat(),0.000001,&flg);
		...
		// Give to the solver A and b, return x, the solution
		auto x = solver.solve(fd.getA(),fd.getB(),SOLVER_PRINT_DETERMINANT);
...
//		std::cout << "Solved component " << i << "  Error: " << ne << "   Symmetric: " << flg << std::endl;
		...
		// copy the solution to grid
		fd.template copy<phi>(x,{0,0,0},szs,gr_ps);
	...
	while (it3.isNext())
	{
		...
		float phi_xy = (gr_ps_v.get<phi>(key.move(y,0))[x] - gr_ps_v.get<phi>(key.move(y,-1))[x])*inv_dy;
		float phi_xz = (gr_ps_v.get<phi>(key.move(z,0))[x] - gr_ps_v.get<phi>(key.move(z,-1))[x])*inv_dz;
		float phi_yx = (gr_ps_v.get<phi>(key.move(x,0))[y] - gr_ps_v.get<phi>(key.move(x,-1))[y])*inv_dx;
		float phi_yz = (gr_ps_v.get<phi>(key.move(z,0))[y] - gr_ps_v.get<phi>(key.move(z,-1))[y])*inv_dz;
		float phi_zx = (gr_ps_v.get<phi>(key.move(x,0))[z] - gr_ps_v.get<phi>(key.move(x,-1))[z])*inv_dx;
		float phi_zy = (gr_ps_v.get<phi>(key.move(y,0))[z] - gr_ps_v.get<phi>(key.move(y,-1))[z])*inv_dy;
	...
	while (it4.isNext())
	{
		...
		float phi_xy = (g_vel.template get<phi>(key.move(y,1))[x] - g_vel.template get<phi>(key.move(y,0))[x])*inv_dy;
		float phi_xz = (g_vel.template get<phi>(key.move(z,1))[x] - g_vel.template get<phi>(key.move(z,0))[x])*inv_dz;
		float phi_yx = (g_vel.template get<phi>(key.move(x,1))[y] - g_vel.template get<phi>(key.move(x,0))[y])*inv_dx;
		float phi_yz = (g_vel.template get<phi>(key.move(z,1))[y] - g_vel.template get<phi>(key.move(z,0))[y])*inv_dz;
		float phi_zx = (g_vel.template get<phi>(key.move(x,1))[z] - g_vel.template get<phi>(key.move(x,0))[z])*inv_dx;
		float phi_zy = (g_vel.template get<phi>(key.move(y,1))[z] - g_vel.template get<phi>(key.move(y,0))[z])*inv_dy;
...
int main(int argc, char* argv[])
{
	...
	// Domain, a rectangle
	Box<3,float> domain({0.0,0.0,0.0},{2.0,1.0,1.0});
