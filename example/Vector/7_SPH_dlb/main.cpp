 ...
 *
 *
 * This example show the classical SPH Dam break simulation with Load Balancing
 * and Dynamic load balancing. With Load balancing and Dynamic load balancing we
 * indicate the possibility of the system to re-adapt the domain decomposition
 * to keep all the processor load and reduce idle time.
 *
 * \htmlonly
 * <a href="#" onclick="hide_show('vector-video-3')" >Simulation video 1</a><br>
 * <div style="display:none" id="vector-video-3">
 * <video id="vid3" width="1200" height="576" controls> <source
 * src="http://openfpm.mpi-cbg.de/web/images/examples/7_SPH_dlb/sph_speed.mp4"
 * type="video/mp4"></video>
 * </div>
 * <a href="#" onclick="hide_show('vector-video-4')" >Simulation video 2</a><br>
 * <div style="display:none" id="vector-video-4">
 * <video id="vid4" width="1200" height="576" controls> <source
 * src="http://openfpm.mpi-cbg.de/web/images/examples/7_SPH_dlb/sph_speed2.mp4"
 * type="video/mp4"></video>
 * </div>
 * <a href="#" onclick="hide_show('vector-video-15')" >Simulation dynamic load
 * balancing video 1</a><br> <div style="display:none" id="vector-video-15">
 * <video id="vid15" width="1200" height="576" controls> <source
 * src="http://openfpm.mpi-cbg.de/web/images/examples/7_SPH_dlb/sph_dlb.mp4"
 * type="video/mp4"></video>
 * </div>
 * <a href="#" onclick="hide_show('vector-video-16')" >Simulation dynamic load
 * balancing video 2</a><br> <div style="display:none" id="vector-video-16">
 * <video id="vid16" width="1200" height="576" controls> <source
 * src="http://openfpm.mpi-cbg.de/web/images/examples/7_SPH_dlb/sph_dlb2.mp4"
 * type="video/mp4"></video>
 * </div>
 * <a href="#" onclick="hide_show('vector-video-17')" >Simulation countour
 * prospective 1</a><br> <div style="display:none" id="vector-video-17"> <video
 * id="vid17" width="1200" height="576" controls> <source
 * src="http://openfpm.mpi-cbg.de/web/images/examples/7_SPH_dlb/sph_zoom.mp4"
 * type="video/mp4"></video>
 * </div>
 * <a href="#" onclick="hide_show('vector-video-18')" >Simulation countour
 * prospective 2</a><br> <div style="display:none" id="vector-video-18"> <video
 * id="vid18" width="1200" height="576" controls> <source
 * src="http://openfpm.mpi-cbg.de/web/images/examples/7_SPH_dlb/sph_back.mp4"
 * type="video/mp4"></video>
 * </div>
 * <a href="#" onclick="hide_show('vector-video-19')" >Simulation countour
 * prospective 3</a><br> <div style="display:none" id="vector-video-19"> <video
 * id="vid19" width="1200" height="576" controls> <source
 * src="http://openfpm.mpi-cbg.de/web/images/examples/7_SPH_dlb/sph_all.mp4"
 * type="video/mp4"></video>
 * </div>
 * \endhtmlonly
 *
 * \htmlonly
 * <img
 * src="http://ppmcore.mpi-cbg.de/web/images/examples/7_SPH_dlb/dam_break_all.jpg"/>
 * \endhtmlonly
 *
 * ## Inclusion ## {#e7_sph_inclusion}
 *
 * In order to use distributed vectors in our code we have to include the file
 * Vector/vector_dist.hpp we also include DrawParticles that has nice utilities
 * to draw particles in parallel accordingly to simple shapes
 *
 ...
 *
 * \f$\frac{dv_a}{dt} = - \sum_{b = NN(a) } m_b \left(\frac{P_a + P_b}{\rho_a
 * \rho_b} + \Pi_{ab} \right) \nabla_{a} W_{ab} + g  \tag{1} \f$
 *
 * \f$\frac{d\rho_a}{dt} =  \sum_{b = NN(a) } m_b v_{ab} \cdot \nabla_{a} W_{ab}
 * \tag{2} \f$
 *
 * \f$ P_a = b \left[ \left( \frac{\rho_a}{\rho_{0}} \right)^{\gamma} - 1
 * \right] \tag{3} \f$
 *
 * with
 *
 * \f$ \Pi_{ab} =  \begin{cases} - \frac {\alpha \bar{c_{ab}} \mu_{ab}
 * }{\bar{\rho_{ab}} } & v_{ab} \cdot r_{ab} > 0 \\ 0 & v_{ab} \cdot r_{ab} < 0
 * \end{cases} \tag{4}\f$
 *
 ...
 * used by Dual-SPH (http://www.dual.sphysics.org/). A summary of the equation
 * and constants can be founded in their User Manual and the XML user Manual.
 *
...
struct ModelCustom {
  ...
  double distributionTol() { return 1.01; }
};
 ...
 *
 * This function implement the formula 3 in the set of equations. It calculate
 * the pressure of each particle based on the local density of each particle.
 *
...
inline void EqState(particles& vd) {
  ...
  while (it.isNext()) {
    ...
    vd.template getProp<Pressure>(a) =
        B * (rho_frac * rho_frac * rho_frac * rho_frac * rho_frac * rho_frac *
                 rho_frac -
             1.0);
 ...
 * defined as
 *
 * \f$ \begin{cases} 1.0 - \frac{3}{2} q^2 + \frac{3}{4} q^3 & 0 < q < 1 \\ (2 -
 * q)^3 & 1 < q < 2 \\ 0 & q > 2 \end{cases} \f$
 *
 ...
 * balancing
 *
 * This function define the gradient of the Cubic kernel function \f$ W_{ab}
 * \f$.
 *
 * \f$ \nabla W_{ab} = \beta (x,y,z)  \f$
 *
 * \f$ \beta = \begin{cases} (c_1 q + d_1 q^2) & 0 < q < 1 \\ c_2 (2 - q)^2  & 1
 * < q < 2 \end{cases} \f$
 *
...
inline void DWab(Point<3, double>& dx,
                 Point<3, double>& DW,
                 double r,
                 bool print) {
  const double qq = r / H;
 ...
 *
 * This function define the Tensile term. An explanation of the Tensile term is
 * out of the context of this tutorial, but in brief is an additional repulsive
 * term that avoid the particles to get too near. Can be considered at small
 * scale like a repulsive force that avoid particles to get too close like the
 * Lennard-Jhonned potential at atomistic level. A good reference is the
 * Monaghan paper "SPH without a Tensile Instability"
 *
...
// Tensile correction
inline double Tensile(double r,
                      double rhoa,
                      double rhob,
                      double prs1,
                      double prs2) {
...
inline double Pi(const Point<3, double>& dr,
                 double rr2,
                 Point<3, double>& dv,
                 double rhoa,
                 double rhob,
                 ...
                 double& visc) {
  const double dot =
      dr.get(0) * dv.get(0) + dr.get(1) * dv.get(1) + dr.get(2) * dv.get(2);
...
inline void calc_forces(particles& vd, CellList& NN, double& max_visc) {
  ...
  while (part.isNext()) {
    ...
    if (vd.getProp<type>(a) != FLUID) {
      // If it is a boundary particle calculate the delta rho based on equation
      // 2 This require to run across the neighborhoods particles of a
      ...
      while (Np.isNext() == true) {
        ...
        if (r2 < 4.0 * H * H) {
          ...
          vd.getProp<drho>(a) +=
              massb * (dv.get(0) * DW.get(0) + dv.get(1) * DW.get(1) +
                       dv.get(2) * DW.get(2));
        }
      ...
      while (Np.isNext() == true) {
        ...
        if (r2 < 4.0 * H * H) {
          ...
          double factor =
              -massb * ((vd.getProp<Pressure>(a) + vd.getProp<Pressure>(b)) /
                            (rhoa * rhob) +
                        Tensile(r, rhoa, rhob, Pa, Pb) +
                        Pi(dr, r2, v_rel, rhoa, rhob, massb, max_visc));
          ...
          vd.getProp<drho>(a) +=
              massb * (v_rel.get(0) * DW.get(0) + v_rel.get(1) * DW.get(1) +
                       v_rel.get(2) * DW.get(2));
        }
 ...
 *
 * This function calculate the Maximum acceleration and velocity across the
 * particles. It is used to calculate a dynamic time-stepping.
 *
...
void max_acceleration_and_velocity(particles& vd,
                                   double& max_acc,
                                   double& max_vel) {
 ...
 * balancing
 *
 * In this example we are using Dynamic time-stepping. The Dynamic time stepping
 * is calculated with the Courant-Friedrich-Lewy condition. See Monaghan 1992
 ...
 *
 * \f$  \delta t_{cv} = min \frac{h}{c_s + max \left| \frac{hv_{ab} \cdot
 * r_{ab}}{r_{ab}^2} \right|} \f$
 *
 *
...
double calc_deltaT(particles& vd, double ViscDtMax) {
  ...
  const double dt_f =
      (Maxacc) ? sqrt(H / Maxacc) : std::numeric_limits<int>::max();
 ...
 * balancing
 *
 * This function perform verlet integration accordingly to the Verlet time
 * stepping scheme
 *
 ...
 *
 * Every N Verlet steps the euler stepping scheme is choosen to avoid
 * instabilities
 *
 ...
 * domain or their density go dangerously out of range. If a particle go out of
 * range is removed from the simulation
 *
...
void verlet_int(particles& vd, double dt) {
  ...
  while (part.isNext()) {
    ...
    if (vd.template getProp<type>(a) == BOUNDARY) {
      ...
      double rhonew =
          vd.template getProp<rho_prev>(a) + dt2 * vd.template getProp<drho>(a);
    ...
    double dy = vd.template getProp<velocity>(a)[1] * dt +
                vd.template getProp<force>(a)[1] * dt205;
    double dz = vd.template getProp<velocity>(a)[2] * dt +
                vd.template getProp<force>(a)[2] * dt205;
    ...
    vd.template getProp<velocity>(a)[0] =
        vd.template getProp<velocity_prev>(a)[0] +
        vd.template getProp<force>(a)[0] * dt2;
    vd.template getProp<velocity>(a)[1] =
        vd.template getProp<velocity_prev>(a)[1] +
        vd.template getProp<force>(a)[1] * dt2;
    vd.template getProp<velocity>(a)[2] =
        vd.template getProp<velocity_prev>(a)[2] +
        vd.template getProp<force>(a)[2] * dt2;
    vd.template getProp<rho>(a) =
        vd.template getProp<rho_prev>(a) + dt2 * vd.template getProp<drho>(a);

    // Check if the particle go out of range in space and in density
    if (vd.getPos(a)[0] < 0.000263878 || vd.getPos(a)[1] < 0.000263878 ||
        vd.getPos(a)[2] < 0.000263878 ||
        vd.getPos(a)[0] > 0.000263878 + 1.59947 ||
        vd.getPos(a)[1] > 0.000263878 + 0.672972 ||
        vd.getPos(a)[2] > 0.000263878 + 0.903944 ||
        vd.template getProp<rho>(a) < RhoMin ||
...
void euler_int(particles& vd, double dt) {
  ...
  while (part.isNext()) {
    ...
    vd.template getProp<velocity>(a)[0] = vd.template getProp<velocity>(a)[0] +
                                          vd.template getProp<force>(a)[0] * dt;
    vd.template getProp<velocity>(a)[1] = vd.template getProp<velocity>(a)[1] +
                                          vd.template getProp<force>(a)[1] * dt;
    vd.template getProp<velocity>(a)[2] = vd.template getProp<velocity>(a)[2] +
                                          vd.template getProp<force>(a)[2] * dt;
    vd.template getProp<rho>(a) =
        vd.template getProp<rho>(a) + dt * vd.template getProp<drho>(a);
 ...
 *
 * This function show how to create a pressure sensor/probe on a set of
 * specified points. To do this from the cell-list we just get an iterator
 * across the neighborhood points of the sensors and we calculate the pressure
 * profile. On the other hand because the sensor is in the processor domain of
 * only one processor, only one processor must do this calculation. We will use
 * the function isLocal to determine which processor contain the probe and only
 * such processor will do the calculation.
 *
 * \warning This type of calculation is suitable if the number of probes is
 * small (like 10) and pressure is not calculated every time step. In case the
 * number of probes is comparable to the number of particles or the pressure is
 * calculated every time-step than we suggest to create a set of "probe"
 * particles
 *
 *
...
int main(int argc, char* argv[]) {
   ...
   *
   * Here we Initialize the library, we create a Box that define our domain,
   * boundary conditions and ghost. We also create a vector that contain two
   * probes to measure pressure
   *
   * \see \ref e0_s_init
   *
   ...
   *
   * Here we define a distributed vector in 3D, we use the particles type that
   * we defined previously. Each particle contain the following properties
   ...
   * must be decomposed and \f$ N_p \f$ is the number of processors. (The
   * concept of sub-sub-domain will be explained leter)
   *
   ...
   *
   * In this part we initialize the problem creating particles. In order to do
   * it we use the class DrawParticles. Because some of the simulation constants
   * require the maximum height \f$ h_{swl} \f$ of the fluid to be calculated
   *  and the maximum fluid height is determined at runtime, some of the
   * constants just after we create the fluid particles
   *
   *  ### Draw Fluid ### {#e7_sph_draw_part_fluid}
   *
   * The Function DrawParticles::DrawBox return an iterator that can be used to
   * create particle in a predefined box (smaller than the simulation domain)
   * with a predefined spacing. We start drawing the fluid particles, the
   * initial pressure is initialized accordingly to the Hydrostatic pressure
   * given by:
   *
   *  \f$ P = \rho_{0} g (h_{max} - z) \f$
   *
   * Where \f$ h_{max} \f$ is the maximum height of the fluid.
   * The density instead is given by the equation (3). Assuming \f$ \rho \f$
   * constant to \f$ \rho_{0} \f$ in the Hydrostatic equation is a good
   * approximation. Velocity is initialized to zero.
   *
   * \see \ref e0_s_vector_inst
   *
   * \htmlonly
   * <img
   * src="http://ppmcore.mpi-cbg.de/web/images/examples/7_SPH_dlb/fluid.jpg"/>
   * \endhtmlonly
   *
  ...
  Box3D fluid_box({dp / 2.0, dp / 2.0, dp / 2.0},
                  {0.4 + dp / 2.0, 0.67 - dp / 2.0, 0.3 + dp / 2.0});
  ...
  h_swl =
      fluid_it.getBoxMargins().getHigh(2) - fluid_it.getBoxMargins().getLow(2);
  ...
  while (fluid_it.isNext()) {
    ...
    // We also initialize the density of the particle and the hydro-static
    // pressure given by
    //
    // rho_zero*g*h = P
    //
    ...
    vd.template getLastProp<Pressure>() =
        rho_zero * gravity * (max_fluid_height - fluid_it.get().get(2));

    vd.template getLastProp<rho>() =
        pow(vd.template getLastProp<Pressure>() / B + 1, 1.0 / gamma_) *
        rho_zero;
   ...
   * ### Draw Recipient ###
   *
   * Here we draw the recipient using the function DrawParticles::DrawSkin. This
   * function can draw a set of particles inside a box A removed of a second box
   * or an array of boxes. So all the particles in the area included in the area
   * A - B - C. There is no restriction that B or C must be included into A.
   *
   * \htmlonly
   * <img
   * src="http://ppmcore.mpi-cbg.de/web/images/examples/7_SPH_dlb/recipient.jpg"/>
   * \endhtmlonly
   *
   * In this case A is the box defining the recipient, B is the box cutting out
   * the internal part of the recipient, C is the hole where we will place the
   * obstacle. Because we use Dynamic boundary condition (DBC) we initialize the
   * density to \f$ \rho_{0} \f$. It will be update over time according to
   * equation (3) to keep the particles confined.
   *
  ...
  // Recipient
  Box3D recipient1({0.0, 0.0, 0.0},
                   {1.6 + dp / 2.0, 0.67 + dp / 2.0, 0.4 + dp / 2.0});
  Box3D recipient2({dp, dp, dp},
                   {1.6 - dp / 2.0, 0.67 - dp / 2.0, 0.4 + dp / 2.0});

  Box3D obstacle1({0.9, 0.24 - dp / 2.0, 0.0},
                  {1.02 + dp / 2.0, 0.36, 0.45 + dp / 2.0});
  Box3D obstacle2({0.9 + dp, 0.24 + dp / 2.0, 0.0},
                  {1.02 - dp / 2.0, 0.36 - dp, 0.45 - dp / 2.0});
   ...
   *  ### Draw Obstacle ###
   *
   * Here we draw the obstacle in the same way we draw the recipient. also for
   * the obstacle is valid the same concept of using Dynamic boundary condition
   * (DBC)
   *
   * \htmlonly
   * <img
   * src="http://ppmcore.mpi-cbg.de/web/images/examples/7_SPH_dlb/obstacle.jpg"/>
   * \endhtmlonly
   *
  ...
  auto obstacle_box =
      DrawParticles::DrawSkin(vd, sz, domain, obstacle2, obstacle1);
   ...
   * ### Load Balancing ###
   *
   * If at this point we output the particles and we visualize where they are
   * accordingly to their processor id we can easily see that particles are
   * distributed unevenly. The processor that has particles in white has few
   * particles and all of them are non fluid. This mean that it will be almost
   * in idle. This situation is not ideal
   *
   * \htmlonly
   * <img
   * src="http://ppmcore.mpi-cbg.de/web/images/examples/7_SPH_dlb/unbalanced_particles.jpg"/>
   * \endhtmlonly
   *
   * In order to reach an optimal situation we have to distribute the particles
   * to reach a balanced situation. To do this we have to set the computation of
   * each sub-sub-domain, redecompose the space and distribute the particles
   * accordingly to this new configuration. To do this we need a model. A model
   * specify how to set the computational cost for each sub-sub-domains (for
   * example it specify if the computational cost to process a sub-sub-domain is
   * quadratic or linear with the number of particles ...). A model look like
   * this.
   *
   * \snippet Vector/7_SPH_dlb/main.cpp model custom
   *
   *  Setting the the computational cost on sub-sub-domains is performed running
   *  across the particles. For each one of them, it is calculated on which
   * sub-sub-domain it belong. Than the function **addComputation** is called.
   * Inside this call we can set the weight in the way we prefer. In this case
   * we set the weight as:
   *
   * \f$ w_v =  4 N_{fluid} + 3 N_{boundary} \f$
   *
   * Where \f$ N_{fluid} \f$ Is the number of fluid particles in the
   * sub-sub-domains and \f$ N_{boundary} \f$ are the number of boundary
   * particles. For example in our ModelCustom we square this number, because
   * the computation is proportional to the square of the number of particles in
   * each sub-sub-domain. A second cycle is performed in order to calculate a
   * complex function of this number (for example squaring).
   *
   * Implicitly the communication cost is given by \f$
   * \frac{V_{ghost}}{V_{sub-sub}} t_s \f$, while the migration cost is given by
   * \f$ v_{sub-sub} \f$. In general\f$ t_s \f$ is the number of ghost get
   * between two rebalance. In this special case where we have two type of
   * particles, we have two different computation for each of them, this mean
   * that fluid particles and boundary particles has different computation cost.
   *
   *  After filling the computational cost based on our model
   * we can decompose the problem in computationally equal chunk for each
   * processor. We use the function **decomposed** to redecompose the space and
   * subsequently we use the function map to redistribute the particles.
   *
   * \note All processors now has part of the fluid. It is good to note that the
   * computationaly balanced configuration does not correspond to the evenly
   * distributed particles to know more about that please follow the video
   * tutorials
   *
   * \htmlonly
   * <a href="#" onclick="hide_show('vector-video-6')" >Dynamic load balancing
   * the theory part1</a><br> <div style="display:none" id="vector-video-6">
   * <video id="vid6" width="1200" height="576" controls> <source
   * src="http://openfpm.mpi-cbg.de/upload/video/dlb-1.mp4"
   * type="video/mp4"></video>
   * </div>
   * <a href="#" onclick="hide_show('vector-video-7')" >Dynamic load balancing
   * the theory part2</a><br> <div style="display:none" id="vector-video-7">
   * <video id="vid7" width="1200" height="576" controls> <source
   * src="http://openfpm.mpi-cbg.de/upload/video/dlb-2.mp4"
   * type="video/mp4"></video>
   * </div>
   * <a href="#" onclick="hide_show('vector-video-8')" >Dynamic load balancing
   * practice part1</a><br> <div style="display:none" id="vector-video-8">
   * <video id="vid8" width="1200" height="576" controls> <source
   * src="http://openfpm.mpi-cbg.de/upload/video/dlb-3.mp4"
   * type="video/mp4"></video>
   * </div>
   * <a href="#" onclick="hide_show('vector-video-9')" >Dynamic load balancing
   * practice part2</a><br> <div style="display:none" id="vector-video-9">
   * <video id="vid9" width="1200" height="576" controls> <source
   * src="http://openfpm.mpi-cbg.de/upload/video/dlb-4.mp4"
   * type="video/mp4"></video>
   * </div>
   * \endhtmlonly
   *
   ...
   * \htmlonly
   * <img
   * src="http://ppmcore.mpi-cbg.de/web/images/examples/7_SPH_dlb/load_balanced_particles.jpg"/>
   * \endhtmlonly
   *
   */
   ...
   * density, than calculate the forces, than we calculate delta time, and
   * finally update position and velocity. After 200 time-step we do a
   * re-balancing. We save the configuration and we calculate the pressure on
   * the probe position every 0.01 seconds
   *
  ...
  while (t <= t_end) {
    ...
    if (write < t * 100) {
      ...
      if (v_cl.getProcessUnitID() == 0) {
        std::cout << "TIME: " << t << "  write " << it_time.getwct() << "   "
                  << v_cl.getProcessUnitID() << "   " << cnt
                  << "   Max visc: " << max_visc << std::endl;
      }
    } else {
      if (v_cl.getProcessUnitID() == 0) {
        std::cout << "TIME: " << t << "  " << it_time.getwct() << "   "
                  << v_cl.getProcessUnitID() << "   " << cnt
                  << "    Max visc: " << max_visc << std::endl;
      }
    }
