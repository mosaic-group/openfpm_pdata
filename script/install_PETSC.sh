...
if [ x"$platform" == x"msys" ]; then
    pacman -Syu mingw-w64-x86_64-metis
    pacman -Syu mingw-w64-x86_64-parmetis
    configure_options="$configure_options --with-single-library=0 --disable-shared --with-windows=graphic=0 --with-x=0 --with-metis=1 --with-parmetis-1 "

    if [ -d "$1/OPENBLAS" ]; then
        configure_options="$configure_options --with-openblas-dir=$1/OPENBLAS/ "
    fi
    mpi_dir=/mingw64
else
