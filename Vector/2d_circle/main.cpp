    ...
    while (part.isNext())
    {
        ...
        while (Np.isNext() == true)
        {
            ...
            if (r2 < 4.0*H*H)
            {
                ...
                //double cfactor = massb/rhob*vd.getProp<type>(b);
...
int main(int argc, char* argv[])
{
	...
	std::cout<<"."<<std::endl;
	Box<2,double> fluid_box({0.0+num_blayers*dp, 0.0+num_blayers*dp}, {(l - num_blayers*dp), (l - num_blayers*dp)});
	...
	while (fluid_it.isNext())
	{
		...
		// and its type.
		if (sqrt((vd.getLastPos()[0]-5)*(vd.getLastPos()[0]-5)+(vd.getLastPos()[1]-5)*(vd.getLastPos()[1]-5))<2)
            {vd.template getLastProp<type>() = FLUID;}
		else
        {vd.template getLastProp<type>() = FLUID_B;}
//        double dist = sqrt((vd.getLastPos()[0]-5)*(vd.getLastPos()[0]-5)+(vd.getLastPos()[1]-5)*(vd.getLastPos()[1]-5))-2;
//        vd.template getLastProp<type>() = dist;
		...
		vd.template getLastProp<Pressure>() = rho_zero * gravity *  (max_fluid_height - fluid_it.get().get(1));
...
/*	auto obstacle_box = DrawParticles::DrawSkin(vd,sz,domain,obstacle2,obstacle1);
	...
	while (t <= t_end)
	{
		...
		// Calc forces
		calc_forces(vd,NN,max_visc); //no need to calculate forces at the moment
		...
		it++;
		if (it < 40) // do not integrate for the moment
...
//        vd.write("fluid_and_bound_after_map_force");
//
//        openfpm_finalize();
//        return 0;
