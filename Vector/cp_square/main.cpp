...
int main(int argc, char* argv[])
{
	...
	while (particle_it.isNext())
	{
		...
		if (abs(dist) < band_width/2.0)
		{
			...
			vd.getLastPos()[1] = y;
			double sdf_guess = 0;
			if (abs(x) > abs(y)) sdf_guess = - sin(2*(abs(x) - A));
			else sdf_guess = - sin(2*(abs(y) - B));
...
//		openfpm_finalize();//
	...
	if(true)
	{
		...
		rdistoptions.polynomial_degree = "quadratic";
	...
	vd.write("square_after_redistancing_after_cleanup");
	openfpm_finalize();
}
